...
Charts MUST have descriptive ARIA labels on their container elements. The system SHALL provide a text summary of the chart's key insight.

(Previously: Charts had no `aria-label` or accessible description)

#### Scenario: Area chart read by screen reader

- GIVEN an area chart displays "Evolución Temporal"
- WHEN a screen reader user navigates to the chart
